...
классов, производных от данного, включая класса, порожденные от 
...
hasDMA & hasDMA::operator=(const hasDMA & hs)
{
	...
	baseDMA::operator=(hs);	// явный вызов операции присваивания, определенной в базовом классе
	delete[] style;			// подготовка к операции new для style
	style = new char[std::istrlen(hs.style) + 1];
	std::strcpy(style, hs.style);
	return *this;
}
...
show(buzz);
inadequate(buzz);
/* В вызове функции show() аргумент rba является ссылкой на объект buzz тина
