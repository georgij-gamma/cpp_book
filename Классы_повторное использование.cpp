...
для параметров шаблонных функций это сделать нельзя. Тем не менее, значения по 
умолчанию нетипизированных параметров можно указывать как для шаблонных классов, 
так и для шаблонных функций.
...
template<class T1> class Pair<T1, int>  {...};	// Специализация, в которой для Т2 указан тип int
...
объявление указывает для Т2 тип int, но оставляет параметр Т1 открытым. Указание всех 
типов приводит к пустым угловым скобкам и получению завершенной явной специализации: */
template<> class Pair<int, int> {...};	// Специализация, в которой для Т1 и Т2 указан тип int
