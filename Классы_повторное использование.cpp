...
template<class T, int n = 0> void funct(T & t) { t * n; }; // нетипизированный параметр int со
/*	Специализации шаблона.								  // значением по умолчанию.
...
то лучше использовать шаблон для получения семейства псевдонимов. Вот как это 
выглядит: */
