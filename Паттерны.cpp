...
Может быть, это именно то, что вам нужно, а может, и нет. Здесь важно то, что прозрачное окружение легко позволяет клиенту 
экспериментировать с разными вариантами без знания подробностей кода, добавляющего декорации. Рамка допускает композицию не более 
чем с одним глифом. Этим она отличается от рассмотренных выше композиций, где родительскому объекту позволялось иметь сколько угодно
потомков. Здесь же заключение чего-то в рамку предполагает, что это «что-то» имеется в единственном экземпляре. Мы могли бы приписать
некоторую семантику декорации более одного объекта, но тогда пришлось бы вводить множество видов композиций с оформлением: оформление
строки, колонки и т. д. Это не улучшит архитектуру, так как у нас уже есть классы для такого рода композиций. Поэтому для композиции 
лучше использовать уже существующие классы, а новые добавлять для оформления результата. Отделение декорации от других видов 
композиции одновременно упрощает классы, реализующие разные элементы оформления, и уменьшает их количество. Кроме того, мы избавляемся 
от необходимости дублировать уже имеющуюся функциональность.

	ПАТТЕРН DECORATOR (ДЕКОРАТОР) (Wrapper (обертка))
 Декоратор — паттерн, структурирующий объекты, абстрагирует отношения между классами и объектами, необходимые для поддержки оформления
с помощью техники прозрачного окружения.
 Назначение:
