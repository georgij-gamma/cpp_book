...
/* где guiFactory — экземпляр класса MotifFactory. Операция CreateScrollBar() возвращает новый экземпляр подходящего подкласса 
ScrollBar, который соответствует нужному варианту оформления, в данном случае Motif. С точки зрения клиентов результат тот же, 
что и при прямом вызове конструктора MotifScrollBar. Но есть и существенное отличие: нигде в коде больше не упоминается имя Motif. 
Объект guiFactory абстрагирует процесс создания полос прокрутки не только для Motif, но и для любых стандартов оформления. Более 
того, guiFactory не ограничивается изготовлением только полос прокрутки и может применяться для производства любых виджетов, 
включая кнопки, поля ввода, меню и т. д. Все это возможно благодаря тому, что MotifFactory является подклассом GUIFactory — 
абстрактного класса, который определяет общий интерфейс для создания глифов-виджетов. В нем есть такие операции, как 
CreateScrollBar и CreateButton, для создания экземпляров различных видов виджетов. Подклассы GUIFactory реализуют эти операции, 
возвращая глифы вроде MotifScrollBar и PMButton, реализующие конкретное оформление и поведение. 
...
guiFactory была инициализирована до того, как начнет использоваться для производства объектов, но после того, как стало известно, 
какое оформление требуется. Когда вариант оформления известен на этапе компиляции, то guiFactory можно инициализировать простым 
присваиванием в начале программы: */
