...
static_cast
reinterpret_cast
/* Пусть High и Low — два класса, переменная ph имеет тип High *, а рl — тип Low *.
Тогда следующий оператор присваивает указатель Low * переменной рl, только если Low
является доступным для High базовым классом (непосредственным или дальним предком): */
pl = dynamic_cast<Low *> ph;
// В противном случае этот оператор присваивает рl нулевой указатель.
