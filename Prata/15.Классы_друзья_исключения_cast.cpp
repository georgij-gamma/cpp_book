...
являются для него друзьями; дружественные отношения нельзя навязать извне.
 ...
 Можно сделать дружественными отдельные члены класса, а не весь класс целиком.
Понадобится следить за порядком различных определений и объявлений: */
class Tv
{
	...
...
а set_chan() — методом этого класса. Это наводит на мысль поместить определение
...
class Probe
{
	...
	friend void sync(Probe & p, const Analyzer & a); // синхронизация p с a
};
class Analyzer
{
	friend void sync(Analyzer & a, const Probe & p); // синхронизация а с р
...
используется объектом cerr) отправляется сообщение вроде "abnormal program termination" 
...
double hmean(double a, double b)
{
    if (a == -b)												// если один аргумент равен другому с
    {														// обратным знаком
...
's' аналогично определению аргумента функции, и если возникшее исключение соответствует
этому объявлению, оно присваивается 's', а затем программа выполняет код внутри фигурных 
скобок. Если программа выполнила все операторы внутри блока try без возникновения 
исключений, она пропускает все блоки catch и переходит к выполнению операторов, следующих 
за обработчиками исключений. То, что оператор continue переносит управление в начало цикла, 
...
if (а == -b)
	throw bad_hmean(a, b);
// Часто после блока try идут несколько блоков catch подряд:
try { 							// начало блока try
	}							// конец блока try
catch (bad_hmean & bg) {...}		// блок catch
catch (bad_gmean & hg) {...}		// блок catch
...
Это процесс и называется раскручиванием стека. После операции throw, как и при возврате
из функции, для всех автоматических объектов, которые находятся в стеке, вызываются 
...
производного объекта. То есть открытое наследование обеспечивает то, что объект
...
Grand * GetOne();			// функция  возвращает указательна базовый класс
...
выполнение. Вместо этого операция dynamic_cast генерирует исключение bad_cast,
производное от класса exception и определенное в заголовочном файле typeinfо.
Поэтому можно применить следующую операцию: */
#include <typeinfo>		// для bad_cast
...
try
{
	Superb & rs = dynamic_cast<Superb &>(rg); // rg — ссылка на объект Grand
	...
}
catch(bad_cast &) { ... };
...
#include <iostream>			// rtti2.cpp  -- использование dynamic_cast, typeid и type_info
#include <cstdlib>
#include <ctime>
#include <typeinfo>
...
int main()
{
    ...
    {
        pg = GetOne();
    // отображает строку, определенную для класса объекта, на который указывает указатель pg:
        cout << "Now processing type " << typeid(*pg).name() << ".\n";
        pg->Speak();
        if( ps = dynamic_cast<Superb *>(pg) )
            ps->Say();
        if( typeid(Magnificent) == typeid(*pg) )	// возвращает булевское значение true, если pg указывает на
            cout << "Yes, you're really magnificent.\n"; // объект Magnificent, и false — в противном случае
    }
    return 0;
}...
...
dynamic_cast.
 ...
 По мнению Бьярне Страуструпа, операция приведения типа в языке С слишком нестрогая: */
struct Data
{
	double data[200];
};
