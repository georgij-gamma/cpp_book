...
транслируется в вызовы функций. Поскольку сложение — операция, выполняемая 
слева направо, первая трансляция дает: */
...
А = В * 2.75; // операция умножения комбинирует значение типа Time со значением типа double
/* Это ограничивает ее применение. Левый операнд — это вызывающий объект. То есть А = В * 2.75;
...
/* Выражение не может соответствовать функции-члену, поскольку 2.75 не является объектом
типа Time. Функция, не являющаяся членом, не вызывается через объект. Вместо этого 
...
неявно как вызывающий объект.
 Оба прототипа соответствуют выражению Т2 + Т3, где Т2 и Т3 — объекты типа Time. 
...
присваивания по умолчанию, определив для класса функцию операции присваивания.
	Сохранение в файле.
Необходимо включить <fstream>, объявить объект ofstream и ассоциировать его с файлом: */
#include <fstream>
ofstream fout;
...
operator имяТипа();	 // где имяТипа — тип, в который преобразуется объект. 
...
/* Можно предоставить либо определение метода, либо определение дружественной 
функции, но не оба сразу. Любая из этих форм делает возможным приведенный ниже код: */
Stonewt jennySt(9, 12);
Stonewt bennySt(12, 8);
Stonewt total;
total = jennySt + bennySt;
/* Также в случае определения конструктора Stonewt (double) каждая из форм 
позволит применять следующие операторы: */
Stonewt jennySt (9, 12);
double kennyD = 17 6.0;
Stonewt total;
total = jennySt + kennyD;
// Но только дружественная функция разрешит такое:
Stonewt jennySt (9, 12);
double pennyD = 14 6.0;
Stonewt total;
total = pennyD + jennySt;
/* Чтобы увидеть, почему это так, необходимо транслировать каждое сложение в 
соответствующий вызов функции. Для начала, код */
total = jennySt + bennySt;
// транслируется в
total = jennySt.operator+(bennySt);	  // функция-член
// либо иначе:
total = operator+(jennySt, bennySt); // дружественная функция
/* В каждом случае типы действительных аргументов соответствуют типам формальных 
аргументов. Кроме того, функция-член вызывается, как это и требуется, объектом
Stonewt. Далее оператор */
total = jennySt + kennyD;
// принимает следующий вид:
total = jennySt.operator+(kennyD); // функция-член
// либо иначе:
total = operator+(jennySt, kennyD); // дружественная функция
/* Здесь снова функция-член вызывается, как положено —
объектом Stonewt. На этот раз в каждом случае один аргумент (kennyD) имеет тип double, 
что вызывает конструктор Stonewt(double) для преобразования double в объект Stonewt.
Наконец, оператор: */
total = pennyD + jennySt;
// превращается в
total = operator+(pennyD, jennySt); // дружественная функция
/* Здесь оба аргумента имеют тип double, что вызывает конструктор Stonewt(double) 
для преобразования их в объекты Stonewt. Однако версия с функцией-членом не сможет 
сложить jennySt с pennyD. После трансляции в вызов функции синтаксис сложения будет 
выглядеть следующим образом: */
total = pennyD.operator+(jennySt); // бессмыслица
/* Этот код не имеет смысла, поскольку только объект класса может вызывать
функцию-член. Компилятор не пытается преобразовать pennyD в объект Stonewt.
Преобразование имеет место только для аргументов функций-членов, а не для 
объектов, на которых они вызываются. Вывод: определение сложения как дружественной
функции упрощает для программы задачу автоматических преобразований типов.
Причина в том, что оба операнда становятся аргументами функции, поэтому 
прототипирование функции применяется в отношении обоих операндов.
	Выбор реализации сложения.
 Первый вариант — определить следующую операцию в виде дружественной функции 
и иметь конструктор Stonewt(double), выполняющий преобразование аргументов 
типа double в аргументы типа Stonewt: */
operator+(const Stonewt &, const Stonewt &)
/* Второй вариант — использовать для перегрузки операции сложения функцию, 
которая явно принимает один аргумент типа double: */
...
/* А следующий оператор явно соответствует функции-члену
operator*(double x, Stonewt & s): */
total = pennyD + jennySt;	  // double + Stonewt
/* Первый вариант менее быстрый и эффективный в плане расхода памяти, но
имеет более короткую запись в исходнике. Второй — более быстрый, но
менее компактнен и следовательно, более подвержен ошибкам программиста. */
