...
/* Конструктор класса 'С' может вызывать только конструкторы класса 'В', а 
конструктор 'В' может вызывать только конструкторы из класса 'А'. В примере 
конструктор 'С' использует значение 'q' и передает значения 'т' и 'п' обратно
конструктору В. Конструктор 'В' использует значение m и передает значение 'п'
конструктору 'А'. Но если Worker будет виртуальным базовым классом, 
автоматическая передача информации работать не будет. 
...
классов, если они написаны с учетом множественного наследования.
 Функция strchr() из библиотеки работы со строками в стиле С: */
...
С и D) и по отдельному подобъекту класса В для каждого невиртуального предка (классов 
...
Слово class не означает, что Тип должен быть классом; это означает только, что
...
когда сin попытается сохранить введенные данные в неподходящем месте. */
...
template<class T1> class Trio<T1, T1*, T1*> {...};		  // Специализация, когда для Т3 и Т2 указан Т1*
...
// Вызов show2(hfi1, hfi2) соответствует следующей специализации:
...
/* Эта функция также является другом для всех специализаций ManyFriend и 
...
Это означает, что тип Currency будет преобразован в double, и каждый раз, когда 
потребуется, преобразованные данные будут переданы в вызов cout. И вот это решение: */
struct Currency
{
	int Dollar;
	int Cents; 
...
конверсия double (и/или в строку) - здесь нет необходимости давать перегрузку (двоичного) оператора*.
