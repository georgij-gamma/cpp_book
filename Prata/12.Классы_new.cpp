...
• конструктор по умолчанию, если не было определено ни одного конструктора; // ничего не 
делает, однако позволяет объявлять массивы и неинициализированные объекты.
• деструктор  по умолчанию, если он не был определен; // ничего не делает.
• конструктор копирования,  если он не был определен; // используют почленное присваивание.
• операция присваивания, если она не была определена; // используют почленное присваивание.
• операция взятия адреса, если она не была определена.// Неявная операция взятия адреса 
возвращает адрес вызывающего объекта (т.е. значение указателя this). В С++11 предлагаются 
еще две специальные функции-члена — конструктор переноса и операция присваивания с переносом.
...
String::String()
{
	...
	str[0] = '\0';		// или str = nullptr; в С++11
}
String::String()
{
	len = 0;
...
force1 + force2 означает такой временный объект. В операторе 1 временный объект 
...
int main()
{
	Act *pt = new Act;	// динамический объект
	{
		...
		...
	}			// Вызывается деструктор для автоматического объекта неявно
	delete pt;	// Вызывается деструктор для динамического объекта явно
	...
}				// Вызывается деструктор для статического объекта неявно
...
//	• Указатель на объект объявляется как обычно; при этом новый объект не создается:
String * glamour;
/*	• Указатель можно инициализировать адресом существующего объекта; при этом 
...
//	• Для получения объекта через указатель применяется операция разыменования (*) :
if (sayings[i] < *first) // сравнение значений объектов
	first = &sayings[i]; // присваивание адреса объекта
	
//	new с размещением.
// placenew.cpp  -- операции new, new с размещением, но без delete
#include <iostream>
#include <string>
#include <new>
using namespace std;
...
explicit, чтобы его нельзя было задействовать для неявных преобразований.
...
то конструктор класса может использовать для инициализации данных-членов синтаксис: */
...
предыдущего конечного узла заносится ссылка на новый конечный узел (nullptr).
...
ссылочную переменную, переданную в метод. */
	...
	Node * temp = front;		/* сохранение местоположения первого элемента (адреса front) для 
последующего удаления. */
	front = front->next;		/* Удалить узел из очереди: сдвиг указателя начала на следующий элемент.
В указатель-член front объекта Queue заносится указатель на следующий узел, адрес которого находится в 
front->next. */
...
class Queue
{
private:	// упреждающиее определения:
...
5. После завершения цикла моделирования программа должна выдать статистический отчет. Для 
