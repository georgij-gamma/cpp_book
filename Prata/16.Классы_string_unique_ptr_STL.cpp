...
приводит к утечке памяти.
 ...
 autoptr, unique_ptr и shared_ptr определяют подобный указателю объект, которому 
...
от films[2] к pwin: */
pwin = films[2];		// films[2] утрачивает права владения
/* Это ведет к тому, что элемент массива films[2] перестает ссылаться па строку.
...
оказывается неприятным сюрпризом.
