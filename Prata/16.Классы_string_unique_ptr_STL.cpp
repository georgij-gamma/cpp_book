...
ps2 = std::move(ps1);					// делает возможным присваивание
ps1 = demo(" and more");
...
/* Вызов функции pushback() работает, поскольку он передает временный объект
unique_ptr, который должен быть присвоен объекту unique_ptr в 'ѵр'. Также 
