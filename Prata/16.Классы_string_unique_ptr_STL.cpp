...
функцию ShowReview(). В отличие от функции f or_each (), цикл for, основанный 
...
• Должна существовать возможность перемещения итератор по элементам контейнера. 
...
достижении итератором этого элемента. Тогда и find_arr(), и find_ll() одинаково 
...
/* Тот факт, что указатели являются итераторами, а алгоритмы построены на основе 
...
которые поддерживают его применение.
 insert_iterator можно использовать для преобразования алгоритма, который
копирует данные, в алгоритм, вставляющий их.
 Эти итераторы принимают тип контейнера в качестве аргумента шаблона и 
фактический идентификатор контейнера — как аргумент конструктора. Таким образом, 
чтобы создать итератор back_insert_iterator для контейнера vector<int> по имени
dice, нужно написать следующий оператор: */
...
типа значений, хранимых в контейнере, а также п — целого и р, q, і и j, представляющих 
итераторы. Требования к последовательностям:
...
void remove(const т & val)		Удаляет все экземпляры ѵаі из списка. Эта функция 
...
затем pop() — для удаления его из очереди.
  priori ty_queue.
 priori ty_queue представляет собой еще один класс адаптера. Он поддерживает те же 
...
X::hasher		Параметр Hash, тип унарного функционального объекта, такой, что если 
				hf имеет тип Hash и к имеет тип Key, то hf(k) имеет тип std::size_t
...
Linear f1;
Linear f2(2.5, 10.0);
double y1 = f1(12.5);	// правой частью является fl.operator() (12.5)
double y2 = f2(0.4);
...
первые два — итераторы, которые задают диапазон контейнера. (Теперь этот подход
уже должен быть хорошо знаком.) Третий аргумент — это итератор, указывающий 
место помещения копии результата. И последний — функтор, который применяется к
...
представления двумерных данных. Например, требуется представить массив из 4 строк и 3 столбцов.
