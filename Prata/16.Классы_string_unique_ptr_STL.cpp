...
Выражение	Описание
а + п		Указывает на n-й элемент после того, на который указывает 'а'
п + а 		То же самое, что а + п
а - п 		Указывает на n-й элемент перед тем, на который указывает 'а'
r += п 		Эквивалентно r = r + п
r -= п 		Эквивалентно r = r - п
а[п] 		Эквивалентно *(а + п)
b - а 		Такое значение п, при котором b = а + п
а < b 		Истинно, если b - а > 0
а > b 		Истинно, если b < а
а >= b 		Истинно, если !(а < Ь)
а <= b 		Истинно, если !(а > Ь)
В этой таблице а и b — значения итератора, а r — переменная или ссылка итератора 
произвольного доступа. Выражения вроде а + п корректны, только если 'а' и а + п лежат 
